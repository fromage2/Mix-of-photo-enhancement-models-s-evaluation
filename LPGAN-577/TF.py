...
psnr_values = np.zeros(29);
ssim_values = np.zeros(29);
time_values = np.zeros(29);


#if len(sys.argv) == 1:
   #print('No arguments passed: image file name expected')
   #sys.exit()
...
def processImg(file_in_name, file_out_name_without_ext,i):
    ...
    enhance_img_file_path = FLAGS['folder_results_DPED'] + enhanced_img_file_name

    psnr_values[i] = psnr(enhance_test_img,original_input_image,0)
    ssim_values[i] = measure.compare_ssim(enhance_test_img,original_input_image,multichannel=True)
    time_values[i] = end - start
    print "PSNR: ",psnr_values
    print "SSIM: ",ssim_values
    print "CPU time: ",time_values
    #try:
...
with tf.Session() as sess:
    ...
    print("\n ######################################################## \n")
    print("Modelo restaurado con exito: ",FLAGS['load_model_path_new'])
    print("\n ######################################################## \n")
    ...
    if len(sys.argv) == 1:
        ...
        i = 0
        file_list_processed_input = [f for f in os.listdir(FLAGS['folder_processed_input']) if os.path.isfile(FLAGS['folder_processed_input'] + f)]
        for f in file_list_processed_input:
                file_name_without_ext = os.path.splitext(f)[0]
                output_file_name = processImg(file_name_without_ext + '.png',file_name_without_ext + '_enhanced',i)
                i = i + 1
